...
const ProposalDetailsPage = ({ mode }) => {
  ...
  useEffect(() => {
    if (leadId && !isEditMode) {
      ...
      const foundLead = storedLeads.find(l => String(l._id) === String(leadId));
      ...
      // Load both leads and customers for reference and dropdown
      const storedLeads = JSON.parse(localStorage.getItem('leads') || '[]');
      const storedCustomers = JSON.parse(localStorage.getItem('customers') || '[]');
      
      // Loop through leads and add the actual customer name from the customerId
      const enrichedLeads = storedLeads.map(lead => {
        if (lead.customerId) {
          const customer = storedCustomers.find(c => c._id === lead.customerId);
          return {
            ...lead,
      ...
      console.log('Loaded leads for dropdown:', enrichedLeads);
      setLeads(enrichedLeads);
      setCustomers(storedCustomers);
      console.log('Loaded customers for reference:', storedCustomers);
    }
  }, [leadId, isEditMode, isCreateMode]);
  ...
  useEffect(() => {
    if (!isCreateMode && id) {
      ...
      const foundProposal = storedProposals.find(p => String(p._id) === String(id));
      
      if (foundProposal) {
        setProposal(foundProposal);
        
        // If we found the proposal, also get its associated lead
        if (foundProposal.lead) {
          const storedLeads = JSON.parse(localStorage.getItem('leads') || '[]');
          const foundLead = storedLeads.find(l => String(l._id) === String(foundProposal.lead));
          if (foundLead) {
            foundProposal.lead = foundLead; // Replace lead ID with lead object
          }
        }
      } else {
        setIsError(true);
  ...
  useEffect(() => {
    if (proposal && !isCreateMode) {
      ...
      // If lead is an object, get just the ID
      if (proposalData.lead && typeof proposalData.lead === 'object') {
        proposalData.lead = proposalData.lead._id;
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.lead) {
      ...
      setValidated(true);
      console.log('Lead selection validation failed');
      return;
    }
    
    console.log('Lead selected for proposal:', formData.lead);
    ...
    try {
      ...
      const storedProposals = JSON.parse(localStorage.getItem('proposals') || '[]');
      console.log('Existing proposals:', storedProposals);
      
      // Get selected lead information
      const storedLeads = JSON.parse(localStorage.getItem('leads') || '[]');
      const selectedLead = storedLeads.find(l => String(l._id) === String(proposalData.lead));
      console.log('Selected lead for proposal:', selectedLead);
      
      // Get customer ID from the lead
      const customerId = selectedLead ? selectedLead.customerId : null;
      console.log('Customer ID for this proposal:', customerId);
      ...
      if (isCreateMode) {
        ...
        const newProposal = {
          ...
          createdBy: {
            _id: userInfo?._id || 'admin',
            name: userInfo?.name || 'Admin User'
          },
          version: 1
        };
        
        console.log('New proposal being created:', newProposal);
        
        // Add to stored proposals
        ...
        setTimeout(() => {
          navigate(`/proposals/${newProposal._id}`, { state: { freshCreated: true } });
        }, 1000);
      } else if (isEditMode && id) {
        // Update existing proposal
        const updatedProposals = storedProposals.map(proposal => {
          if (String(proposal._id) === String(id)) {
            return {
              ...proposal,
              ...proposalData,
              _id: proposal._id, // Ensure ID doesn't change
                        ...
                        <Form.Group controlId="lead" className="mb-3">
                          ...
                          {isCreateMode ? (
                            ...
                            <Form.Control
                              ...
                              onChange={(e) => {
                                handleChange(e);
                                console.log('Lead selection changed to:', e.target.value);
                              }}
                              required
                              disabled={!!leadId}
                              ...
                              {leadId && lead ? (
                                ...
                                leads.map((leadOption) => (
                                  <option key={leadOption._id} value={leadOption._id}>
                                    {leadOption.name} {leadOption.email && `(${leadOption.email})`}
                                    {leadOption.customerName && ` - Customer: ${leadOption.customerName}`}
                                  </option>
                                ))
                              ) : null}
                            </Form.Control>
                            ...
                            {leads && leads.length === 0 && !isLoading && (
                              <Alert variant="info" className="mt-2">
                                No leads found. <Link to="/leads/create">Create a new lead</Link> first.
                              </Alert>
                            )}
                            {leads && leads.length > 0 && (
                              <div className="mt-2 text-muted">
                                <small>Available leads: {leads.length}</small>
                              </div>
                            )}
                          </>
                          ) : (
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    const updatedProposals = storedProposals.map(p => {
                      if (String(p._id) === String(proposal._id)) {
